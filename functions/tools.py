...
class Tools:
    ...
    async def compress(self, dl, out):
        cmd = f'''{Var.FFMPEG} -i """{dl}""" -metadata "Encoded By"="@AnimeHarbour" -map 0:v -map 0:a -map 0:s -c:v libx264 -x265-params 'bframes=8:psy-rd=1:ref=3:aq-mode=3:aq-strength=0.8:deblock=1,1' -pix_fmt yuv420p -crf {Var.CRF} -c:a libopus -b:a 32k -ac 2 -ab 32k -vbr 2 -level 3.1 -threads 2 -preset veryfast """{out}""" -y'''
        process = await asyncio.create_subprocess_shell(
